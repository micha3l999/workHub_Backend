...
async function getFavoriteServices(req, res) {

  try {

    const customerFavorites = await Customer.findOne({ _id: req.user.customerId }, { favServices: 1 }, { lean: true })
      .populate({path: "favServices", select: "mainImage price rating title duration sellerUser", populate: { path: "sellerUser", select: "name lastName" } });

    if (!customerFavorites) {
      return sendNotFound(res);
    }

    sendSucess(res, customerFavorites.favServices);
