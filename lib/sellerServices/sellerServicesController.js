...
const servicesForCustomer = async (req, res) => {
  try {
    const { sellerUser, subcategoryId, sortBy, sortOrder } = req.query;
    let sortByField;

    switch (sortBy) {
      case CONFIG.SORT.TITLE:
        sortByField = {
          title: sortOrder
        };
        break;
      case CONFIG.SORT.PRICE:
        sortByField = {
          price: sortOrder
        };
        break;
      case CONFIG.SORT.DATE:
        sortByField = {
          _id: sortOrder
        };
        break;
      case CONFIG.SORT.RATING:
        sortByField = {
          rating: sortOrder
        };
        break;
    ...
    let services = await SellerService.find({ subcategoryId, sellerUser: { $ne: sellerUser } })
      .populate({ path: "sellerUser", populate: { path: "sellerId", select: "busy rating" },  }).sort(sortByField);
...
const getSellerServiceById = async (req, res) => {
  ...
  try {
    ...
    const queryFavorite = Customer.findOne({ _id: req.user.customerId, favServices: sellerServiceId }, {}, { lean: true });
    const querySellerService = SellerService.findOne({ _id: sellerServiceId }).populate({ 
      path: "sellerUser", 
      populate: { 
        path: "sellerId", 
    ...
    const isFavorite = results[0] ? true : false;
    ...
    res.status(200).json({ sellerService, countServices: countServices[0].n, isFavorite });
  } catch(error) {
    res.status(500).json({error: error.message});
    logger.error(error);
  }
...
const preCheckoutInformation = async (req, res) => {
  ...
  try {
    ...
    const service = await SellerService.findOne({ _id }).populate({ path: "sellerUser", select: "name lastName sellerId", populate: { path: "sellerId", select: "busy" } }).populate('subcategoryId');
