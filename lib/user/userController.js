...
const getUserDataHome = async (req, res) => {
    try {
        ...
        const activeServices = await HiredServices.find( { customerUserId: userData._id, status: 0 } ).populate({ path: 'sellerServiceId', select: 'mainImage sellerUser subcategoryId duration description title'})
                .populate('sellerId');
